...
const Chatwithadmin = () => {
  ...
  const getcustomerlist = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/vendorChatlist`,
        data,
        options
      )
      .then((res) => {
  ...
  const sendmessage = (e) => {
    ...
    if (fileimage) {
      ...
      formData.append("venderId", customerid);
    ...
    {FilteredCustomerList?.length === 0 ? (
      ...
      FilteredCustomerList?.map((chatlist, index) => (
        ...
        >
          <div
            onClick={() => {
              getvendormessage(chatlist?.venderId);
              idcheck(chatlist?.venderId);
            }}
