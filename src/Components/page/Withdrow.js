...
const Withdrow = () => {
  ...
  const getwithdrow = () => {
    axios
      .get(
        `${process.env.REACT_APP_API_KEY}admin/api/vendorWithdraw_requestList`
      )
      .then((res) => {
        let filteredData = [];

        if (valueof == "0") {
          filteredData = res.data.data.filter((item) => item.status == "0");
        } else if (valueof == "1") {
          filteredData = res.data.data.filter((item) => item.status == "1");
        } else if (valueof == "2") {
          filteredData = res.data.data.filter((item) => item.status == "2");
        } else {
  ...
  const renderProductData = (data, index) => {
    ...
    return (
      <tr>
        ...
        <td>${data?.amount}</td>
        <td>
          {data?.venderId?.firstName ? (
            <a href="javascript:">{data?.venderId?.firstName}</a>
          ) : (
        ...
        <td>
          <div class="d-flex justify-content-center">
            <Link
              onClick={(e) => {
                secureLocalStorage.setItem("venderIds", data?.venderId?._id);
