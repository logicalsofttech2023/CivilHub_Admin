...
const Addnewsize = () => {
  ...
  const addsize = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/createSize`,
        formData,
        options
      )
      .then((res) => {
  ...
  let getsizeelist = () => {
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/sizeList`)
      .then((res) => {
  ...
  const handleFilter = (event) => {
    ...
    const result = sizeelist.filter((item) =>
      item?.subSubcatetoryId?.english_sub_subcategory_name
        ?.toLowerCase()
        .includes(searchTerm)
    );
...
<img className="img-fluid" alt="" src="https://6valley.6amtech.com/storage/app/public/brand/2023-06-13-6488153b45156.png" />
        ...
        <td className="text-center">
          {data?.subSubcatetoryId?.english_sub_subcategory_name}
        </td>
                          ...
                          >
                            ...
                            {categorylist?.map((type) => (
                              <option key={type?._id} value={type?._id}>
                                {type?.english_sub_subcategory_name}
                              </option>
                            ))}
                          </select>
                        </div>
                          ...
                          >
                            <thead className="thead-light thead-50 text-capitalize">
                              <tr>
                                ...
                                {/* <th>Brand Logo</th> */}
                                <th>Size</th>
                                <th className="text-center">Subsubcategory</th>
                            ...
                            <tbody>
                              {filteredsizeList
                                ...
                                .map((sizeelist, index) =>
                                  renderBrandData(sizeelist, index)
                                )}
                            </tbody>
                          </table>
                        ) : (
