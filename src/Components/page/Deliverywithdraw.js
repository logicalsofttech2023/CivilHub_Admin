...
const Deliverywithdraw = () => {
  ...
  const getwithdrow = () => {
    axios
      .get(
        `${process.env.REACT_APP_API_KEY}admin/api/driverWithdraw_requestList`
      )
      .then((res) => {
        let filteredData = [];
  ...
  const renderProductData = (data, index) => {
    ...
    return (
      <tr>
        ...
        <td>
          {data?.driverId?.fname ? (
            <a href="javascript:">{data?.driverId?.fname}</a>
          ) : (
        ...
        <td>
          <div class="d-flex justify-content-center">
            <Link
              onClick={(e) => {
                secureLocalStorage.setItem("venderIds", data?.driverId?._id);
                secureLocalStorage.setItem("withdrawids", data?._id);
              }}
              class="btn btn-outline-info btn-sm square-btn"
              title="View"
              to="/deliverywithdrawreview"
            >
