...
const Addnewbrands = () => {
  ...
  const [branddlist, setbranddlist] = useState();
  ...
  useEffect(() => {
    setfilteredbrandList(branddlist);
  }, [branddlist]);
  ...
  const addsize = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/createBrand`,
        formData,
        options
      )
      .then((res) => {
  ...
  let getbranddlist = () => {
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/brandList`)
      .then((res) => {
        setcount(res?.data?.data?.length);
        setbranddlist(res.data.data);
      })
      .catch((error) => {
  ...
  const handleFilter = (event) => {
    ...
    const result = branddlist?.filter((item) =>
      item.categoryId?.category_englishName?.toLowerCase().includes(searchTerm)
    );
  ...
  const renderBrandData = (data, index) => {
...
<img className="img-fluid" alt="" src="https://6valley.6amtech.com/storage/app/public/brand/2023-06-13-6488153b45156.png" />
        ...
        <td style={{ whiteSpace: "break-spaces" }}>{data?.brand.join(", ")}</td>
                          ...
                          >
                            ...
                            <tbody>
                              {filteredbrandList
                                ...
                                .map((branddlist, index) =>
                                  renderBrandData(branddlist, index)
                                )}
                            </tbody>
                          </table>
                        ) : (
...
<p class="mb-0 order-stats__subtitle">No Category found</p>
                        ...
                        <div className="d-flex justify-content-center mt-4">
                          {filteredbrandList?.length > itemsPerPage && (
                            <Pagination
                              activePage={activePage}
                              itemsCountPerPage={itemsPerPage}
                              totalItemsCount={filteredbrandList.length}
