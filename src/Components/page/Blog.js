...
const Blog = () => {
  ...
  let getbannerlist = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_KEY}admin/api/admin_all_blogs`,
        options
      )
      .then((res) => {
  ...
  const renderCategoryData = (categorydata, index) => {
    ...
    return (
      <tr key={index}>
        ...
        <td>{categorydata?.description.replace(/<\/?[^>]*>?/g, "").slice(0, 20)}</td>
        <td>{categorydata?.other_name}</td>
        <td>{categorydata?.date}</td>
